...
for jj=1:9
    ...
    %% Chain
    eqn1=diff(X1,t) ==FX1;
    eqn2=diff(X2,t) ==FX2+K1*b2*c2*(X1-X2);
    ...
    odeFunction(FAll,XAll,Par,'File','odeMotifChain');
    tf=10000;
    tspan = [0 tf];
    RMS123=[];
    ...
    for k1=-5:1/128:5
        ...
        y0 = rand(1,N);
        [tk,y,te,ye,ie] = ode45(@(t,y) odeMotifChain(t,y,Parval), tspan, y0,options);
    ...
    title(strcat('$b_',num2str(2),'^',num2str(coli),'\rightarrow  c_',num2str(2),'^',num2str(rowi),'$' ...
        ,' ',',$b_',num2str(3),'^',num2str(colj),'\rightarrow  c_',num2str(3),'^',num2str(rowj),'$'),'interpreter','latex','FontSize',25)
...
exportgraphics(gcf,'ChainStaticRossler.eps','ContentType','vector')
