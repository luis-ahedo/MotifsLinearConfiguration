...
for ii=1:9
    ...
    %% Chain
    eqn1=diff(X1,t) ==FX1;
    eqn2=diff(X2,t) ==FX2+ks*b*c*(X1-X2)-b2*h1(t);
    ...
    odeFunction(F,varS,Par,'File','odeMotifs');
    tf=10000;
    tspan = [0 tf];
    ...
    for k1=-5:1/128:5
        ...
        y0 = rand(1,N+1)';
        [tk,y,te,ye,ie] = ode45(@(t,y) odeMotifs(t,y,Parval), tspan, y0,options);
...
exportgraphics(gcf,'ChainStaticDynamicscRossler.eps','ContentType','vector')
